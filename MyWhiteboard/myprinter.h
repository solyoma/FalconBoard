...
struct MyPrinterData
{
    ...
    QRectF printArea = QRectF(0, 0, 2480.0, 3500.0);    // get from printer:printable area in device pixels (dots)
    int flags;
};
...
class MyPrinter
{
    ...
    {
        int pageNumber;
        QRect area;             // screen area for page set from screenWidth and _data.printArea
        QVector<int> yindices;   // of drawables to print on this page (ordered by y, z-order [screenshots:0, other:1] then x)
    ...
    bool _PrintPage(int page);  // using _history
